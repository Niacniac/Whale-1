        ...
        for (int searchDepth = 1; searchDepth <= 220; searchDepth++)
        {
            ...
            else
            {
                ...
                if (thread == 0)
                {
                    ...
                    Console.WriteLine($"info depth {threadWorkerDatas[thread].currentDepth} score cp {threadWorkerDatas[thread].bestEval * 100 / 208} nodes {totalNodes} nps {nodesPerSecond} hashfull {Convert.ToInt32(perMillTTfull)} time {totalElapsedTime.Milliseconds + totalElapsedTime.Seconds * 1000 + totalElapsedTime.Minutes * 60 * 1000} pv {pvLineName}");
                    // Update diagnostics
